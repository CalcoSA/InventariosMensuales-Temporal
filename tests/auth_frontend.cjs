...
const draftKey = 'inventario-mensual-v1-PDV-2026-09-09-BEBIDAS';
...
function world(storageAvailable = true) {
  ...
  async function request(url, options = {}) {
    ...
    if (status === 204 && url === '/auth/activity') {
      assert.equal(options.headers['X-Monthly-Request'], '1');
...
(async () => {
  {
    ...
    const fresh = w.tab();
    vm.runInContext(fs.readFileSync('app/static/js/monthly.js', 'utf8'), fresh.context);
  ...
  }
  {
    ...
    assert.ok([...w.entries.keys()].every(k => k === draftKey || k.startsWith('inventario-mensual-auth-v1-')));
  }
  {
    const w = world(), a = w.tab();
    await a.window.fetch('/api/guardarInventario', {method: 'POST', body: '{}'});
    assert.equal(w.calls.at(-1).options.headers.get('X-Monthly-Request'), '1');
    await w.advance(1200);
    await a.window.fetch('/api/obtenerProductos');
    assert.equal(w.entries.get(draftKey), draft);
